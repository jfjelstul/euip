...
template_DDY <- create_template(
  ...
  names = c("directorate_general", "member_state", "year", "decision_stage")
)

# collapse
decisions_DDY <- decisions %>%
  dplyr::group_by(directorate_general, member_state, year, decision_stage) %>%
  dplyr::summarize(
    count_decisions = dplyr::n()
  ) %>% dplyr::ungroup()

# merge into template
decisions_DDY <- dplyr::left_join(template_DDY, decisions_DDY, by = c("directorate_general", "member_state", "year", "decision_stage"))
...
# arrange
decisions_DDY <- dplyr::arrange(decisions_DDY, year, directorate_general_ID, member_state_ID, decision_stage_ID)
...
template_DDY_D <- create_template(
  ...
  decision_stages$decision_stage,
  names = c("directorate_general", "member_state", "year", "case_type", "decision_stage")
)

# collapse
decisions_DDY_D <- decisions %>%
  dplyr::group_by(directorate_general, member_state, year, case_type, decision_stage) %>%
  dplyr::summarize(
    count_decisions = dplyr::n()
  ) %>% dplyr::ungroup()

# merge
decisions_DDY_D <- dplyr::left_join(template_DDY_D, decisions_DDY_D, by = c("directorate_general", "member_state", "year", "case_type", "decision_stage"))
...
decisions_DDY_D <- dplyr::left_join(decisions_DDY_D, decision_stages, by = "decision_stage")

# arrange
decisions_DDY_D <- dplyr::arrange(decisions_DDY_D, year, directorate_general_ID, member_state_ID, case_type_ID, decision_stage_ID)
