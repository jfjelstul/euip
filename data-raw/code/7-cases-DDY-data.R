...
# merge in department data
cases_ddy_d <- dplyr::left_join(cases_ddy_d, departments, by = "department")

# merge in case type data
cases_ddy_d <- dplyr::left_join(cases_ddy_d, case_types, by = "case_type")

# arrange
cases_ddy_d <- dplyr::arrange(cases_ddy_d, year, department_id, member_state_id, case_type_id)
